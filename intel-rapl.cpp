...
static std::vector<double> rapl_perf(int core,int choice, int ms_time) {
	...
	for (long l = 0; l < MAX; l++)
	{
		deq.insert(deq.end(),Shape(l,l+1));
	}*/
	...
	//std::cout <<  std::chrono::duration_cast<std::chrono::milliseconds>(t).count()  << std::endl;
	...
	/*for(j=0;j<total_packages;j++) {
		...
		for(i=0;i<NUM_RAPL_DOMAINS;i++) {
			...
			if (fd[i][j]!=-1) {
				read(fd[2][0],&value,8);
				close(fd[2][0]);
				return ((double)value*scale[0]+(double)value*scale[1]+(double)value*scale[2]+(double)value*scale[3]);
	...
	//return ((double)value*scale[0]+(double)value2*scale[1]+(double)value3*scale[2]+(double)value4*scale[3]);
	/*printf("%s",rapl_domain_names[0]);
	printf("\n");
	printf("%lf",(double)value*scale[0]);
	printf("\n");
	printf("%s",rapl_domain_names[2]);
	printf("\n");
	printf("%lf",(double)value3*scale[2]);
	printf("\n");
	printf("%s",rapl_domain_names[3]);
	printf("\n");
	printf("%lf",(double)value4*scale[3]);
	printf("\n");*/
